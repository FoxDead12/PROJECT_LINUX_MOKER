...
void chamar_lock_do_kernel() {
  printf("[KERNEL CALL] -> A pedir o PCP Mutex...\n");
  syscall(474);
}

void chamar_unlock_do_kernel() {
  printf("[KERNEL CALL] -> A libertar o PCP Mutex...\n");
  syscall(475);
}
...
void* thread_high(void* arg) {
  ...
  chamar_lock_do_kernel(); // Aqui o teu PCP no kernel deve fazer o BOOST à LOW!
...
int main() {
  ...
  printf("--- INICIO DO TESTE PCP MUTEX ---\n");
